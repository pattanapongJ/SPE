...
class PurchaseOrder(models.Model):
    ...
    purchase_manual_currency_inverse_rate = fields.Float('Inverse Rate', digits=0,default=1,
                                                         group_operator="avg",
    ...
    def onchange_manual_currency_rate_active(self):
        if self.purchase_manual_currency_rate_active and self.purchase_manual_currency_inverse_rate == 1:
            ...
            self.purchase_manual_currency_rate = _rate
            self._inverse_manual_rate()
        elif not self.purchase_manual_currency_rate_active:
            self.purchase_manual_currency_rate = 1.0
            self._inverse_manual_rate()

    @api.depends('purchase_manual_currency_inverse_rate')
    def _compute_manual_rate(self):
        for currency_rate in self:
            if not currency_rate.purchase_manual_currency_inverse_rate:
                currency_rate.purchase_manual_currency_inverse_rate = 1.0
            currency_rate.purchase_manual_currency_rate = 1.0 / currency_rate.purchase_manual_currency_inverse_rate

    @api.onchange('purchase_manual_currency_rate')
    def _inverse_manual_rate(self):
        for currency_rate in self:
            if not currency_rate.purchase_manual_currency_rate:
                currency_rate.purchase_manual_currency_rate = 1.0
            currency_rate.purchase_manual_currency_inverse_rate = 1.0 / currency_rate.purchase_manual_currency_rate
    ...
    def _prepare_invoice(self):
        ...
        _invoice_value.update({
            ...
            'manual_currency_inverse_rate': self.purchase_manual_currency_inverse_rate,
        })
