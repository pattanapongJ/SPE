...
class account_payment(models.TransientModel):
    ...
    def _compute_amount(self):
        return super(account_payment, self)._compute_amount()
        # for wizard in self:
...
class AccountPayment(models.Model):
    ...
    def _prepare_move_line_default_vals(self, write_off_line_vals=None):
        """
        Adjust journal line values using a manual currency rate when active.
        """
        ...
        if self.manual_currency_rate_active and self.manual_currency_rate > 0:
            for line in result:
                # Skip adjustments if payment currency matches company currency
                if self.company_id.currency_id.id == self.currency_id.id:
                    continue

                # Adjust values if currencies differ
                amount_currency = line.get('amount_currency', 0.0)
                if amount_currency:
                    _amount = self.company_currency_id.round(abs(amount_currency) / self.manual_currency_rate)
                    if line.get('debit'):
                        line['debit'] = _amount
