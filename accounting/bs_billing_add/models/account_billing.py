...
class AccountBillings(models.Model):
    ...
    start_date = fields.Date(
        string="Start Invoice Date",
        default=lambda self: fields.Date.context_today(self),
    )
    end_date = fields.Date(
        string="End Invoice Date", default=lambda self: fields.Date.context_today(self)
    )
    ...
    def _compute_amount(self):
        ...
        for rec in bills:
            rec.total_bill_amount = sum(
                float(line["total"]) for line in rec.billing_line_ids
            )
