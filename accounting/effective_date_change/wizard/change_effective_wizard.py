        ...
        for picking in self.env['stock.picking'].browse(self._context.get('active_ids', [])):
            ...
            if current_picking_id == 'internal':
                ...
                if current_picking_id == 'incoming' and int(purchase_orders_ids.currency_id) != system_default_currency:
                    rate = float(self.env['res.currency.rate'].search([('currency_id', '=', int(purchase_orders_ids.currency_id)), ('name', '=', selected_date.strftime('%Y-%m-%d'))]).rate)
                    if rate == 0.0:
                        ...
                        for product in picking.move_ids_without_package:
                            if product.product_tmpl_id.categ_id.property_cost_method == 'average':
                                ...
                                for valuation in valuations:
                                    ...
                                    res_id = 'product.product,' + str(product.product_tmpl_id.id)
                                ...
                                if len(valuation) > 1:
                                    ...
                                    highest_valuation_id = max(ids)
                                    for loop in valuation:
                                        if loop.id == highest_valuation_id:
                                            res_id = 'product.product,' + str(product.product_tmpl_id.id)
                                            self.env.cr.execute(
                                                "UPDATE ir_property SET value_float = (%s) WHERE res_id = (%s)",
                                                [loop.unit_cost, res_id])
            else:
                pass
            ...
            def update_journal_name(selected_prefix, picking_name):
                ...
                max_sequence_number = max(seq_numbers, default=0) + 2
