...
class BlanketOrder(models.Model):
    ...
    def _compute_sale_count(self):
        for blanket_order in self:
            blanket_order.sale_count = len(blanket_order._get_sale_orders().filtered(lambda order: order.state != 'cancel'))
    ...
    def action_confirm(self):    
        ...
        for order in self:
            if not order.sale_type_id.sa_sequence_id:
...
class BlanketOrderLine(models.Model):
    ...
    def _get_display_price(self, product):
        self.ensure_one()
        order = self.order_id
        order.ensure_one()  # ป้องกันปัญหา order_id มีหลายตัว
