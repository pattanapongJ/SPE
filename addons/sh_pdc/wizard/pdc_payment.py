...
class PDC_wizard(models.Model):
    ...
    def compute_account_moves(self):
        for rec in self:
            ...
            domain = [('partner_id','=',rec.partner_id.id),('payment_state','!=','paid'),('amount_residual','!=',0.0),('state','=','posted')]
            if rec.payment_type == 'receive_money':
                domain.extend([('move_type', '=','out_invoice')])
            else:
                domain.extend([('move_type', '=','in_invoice')])
            moves = rec.env['account.move'].search(domain)
            rec.account_move_ids = moves.ids
