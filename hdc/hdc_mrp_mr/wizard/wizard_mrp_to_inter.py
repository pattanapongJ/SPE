...
class WizardMRPMRInterTransferLine(models.TransientModel):
    _name = 'wizard.mrp.mr.intertransfer.line'
    _description = 'order line'

    wizard_mrp_to_int_ids = fields.Many2one(
        comodel_name="wizard.mrp.mr.intertransfer",
...
class WizardMRPMRInterTransfer(models.TransientModel):
    _name = 'wizard.mrp.mr.intertransfer'
    ...
    order_line_ids = fields.One2many(
        comodel_name="wizard.mrp.mr.intertransfer.line",
        inverse_name='wizard_mrp_to_int_ids',
        string="Order Line",
    )
    ...
    def generate_int(self):
        if self.order_line_ids:
            ...
            if len(order_line_ids) > 0:
                ...
                if self.mr_id.request_type.is_claim == True:
                    if self.mr_id.state == "approved":
                        self.mr_id.state = "approved"
                    else:
                        self.mr_id.state = "delivery"
                else:
                    self.mr_id.state = "delivery"
                return action
