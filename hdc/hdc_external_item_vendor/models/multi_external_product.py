...
class MultiExternalVendor(models.Model):
    _name = 'multi.external.vendor'
    _description = "Multi External Item Vendor"
    ...
    def create(self, vals):
        res = super(MultiExternalVendor, self).create(vals)     
        if res.name == False:
            res.name = 0
        return res
    ...
    def write(self, vals):
        if vals['name'] == False:
            vals['name'] = 0
        res = super(MultiExternalVendor, self).write(vals)
        return res