...
class Currency(models.Model):
    ...
    def _convert_nn_en(self, val):
        ...
        for (dcap, dval) in ((k, 10 + (10 * v)) for (v, k) in enumerate(tens_en)):
            if dval + 10 > val:
                if val % 10:
                    return dcap + ('-' + to_9_en[val % 10] if to_9_en[val % 10] != 'Zero' else '')
                return dcap
    ...
    def amount_to_text_en(self, number, currency):
        ...
        start_word = self.amount_to_text(int(number_parts[0]))
        end_word = self.english_number(int(number_parts[1]))
        cents_number = int(number_parts[1])
        if currency == 'THB':
            units_name = 'Baht'
            cents_name = 'Satang'
        if currency == 'JYP':
        ...
        print(cents_number , "cents_number")

        if end_word == 'Zero':
            return ' '.join(filter(None, [start_word, (start_word) and 'and', 'Whole']))
        else:
            return ' '.join(filter(None, [start_word, (start_word) and 'and', end_word, cents_name]))
