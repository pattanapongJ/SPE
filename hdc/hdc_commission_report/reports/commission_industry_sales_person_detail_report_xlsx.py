...
class CommissionIndustrySalesPersonDetailXlsx(models.AbstractModel):
    _name = 'report.commission_industry_sales_person_detail_report_xlsx'
    _description = 'report.commission_industry_sales_person_detail_report_xlsx'
    ...
    def generate_xlsx_report(self, workbook, data, partners):
        ...
        sheet.merge_range(merge_row, 'D', head_cell_format)

        merge_row = 'I' + str(row + 1) + ':I' + str(row + 2)
        sheet.merge_range(merge_row, 'D1', head_cell_format)

        merge_row = 'J' + str(row + 1) + ':J' + str(row + 2)
        sheet.merge_range(merge_row, 'D2.5', head_cell_format)

        merge_row = 'K' + str(row + 1) + ':K' + str(row + 2)
        sheet.merge_range(merge_row, 'S', head_cell_format)

        merge_row = 'L' + str(row + 1) + ':L' + str(row + 2)
        sheet.merge_range(merge_row, '10%', head_cell_format)

        merge_row = 'M' + str(row + 1) + ':M' + str(row + 2)
        sheet.merge_range(merge_row, '5%', head_cell_format)

        merge_row = 'N' + str(row + 1) + ':N' + str(row + 2)
        sheet.merge_range(merge_row, '2%', head_cell_format)

        sheet.write(row,14, 'NO', head_cell_format_up)
        sheet.write(row+1,14, 'COM', head_cell_format_down)

        merge_row = 'P' + str(row + 1) + ':P' + str(row + 2)
        sheet.merge_range(merge_row, 'อื่น ๆ', head_cell_format)

        sheet.write(row,16, 'รวม รับ-ก่อน', head_cell_format_up)
        sheet.write(row+1,16, 'ภาษี', head_cell_format_down)

        merge_row = 'R' + str(row + 1) + ':R' + str(row + 2)
