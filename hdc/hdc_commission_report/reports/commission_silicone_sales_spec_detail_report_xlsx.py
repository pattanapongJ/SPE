...
class CommissionSiliconeSalesSpecDetailXlsx(models.AbstractModel):
    _name = 'report.commission_silicone_sales_spec_detail_report_xlsx'
    _description = 'report.commission_silicone_sales_spec_detail_report_xlsx'
    ...
    def generate_xlsx_report(self, workbook, data, partners):
        ...
        sheet.set_margins(left=0.22, right=0.22, top=0.55, bottom=0.3)
        ...
        sheet.merge_range(merge_row, 'No Com', head_cell_format)
        
        merge_row = 'L' + str(row + 1) + ':L' + str(row + 2)
        sheet.merge_range(merge_row, 'พนักงานขาย', head_cell_format)
        ...
        for item in payment_lines:
            ...
            if invoice_id_current != invoice_id_old:
                ...
                if commission_types[0].id in item.product_id.commission_type.ids or commission_types[1].id in item.product_id.commission_type.ids:
                    ...
                    total_amount_no_com = item.total_amount_untax
                    total_amount_no_com_sum = total_amount_no_com_sum + total_amount_no_com
                    total_amount_no_com_sum_all = total_amount_no_com_sum_all + total_amount_no_com
                ...
                data_list_1_2 = [
                            ...
                            '',
                            '',
                            item.invoice_id.user_id.name or '',
                            ]
