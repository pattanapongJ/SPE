...
class CommissionRinnaSalesPersonDetailXlsx(models.AbstractModel):
    _name = 'report.commission_rinna_sales_person_detail_report_xlsx'
    _description = 'report.commission_rinna_sales_person_detail_report_xlsx'
    ...
    def generate_xlsx_report(self, workbook, data, partners):
        ...
        sheet.merge_range('D2:F2', 'คอมมิชชั่น เดือน' + ' ' + str(text_month_commission_date) + ' ' + str(year_commission_date), top_cell_format)
        data_list_header = [
                        'CODE', 
        ...
        sheet.write_row(2,3, data_list_header,top_cell_format)
        sheet.merge_range('H3:I3', 'ทีม' + team_id.name, top_cell_format)
        ...
        sheet.merge_range('G2:H2', '( รับชำระ' + ' ' + str(day_receipted_date_form) + '/' + str(month_receipted_date_form) + '/' + str(year_receipted_date_form) + ' ถึง ' + str(day_receipted_date_to) + '/' + str(month_receipted_date_to) + '/' + str(year_receipted_date_to) + ' )', top_cell_format)
        
        row = 5
        sheet.set_row(row, 15)
        sheet.set_row(row+1, 15)

        merge_row = 'A' + str(row + 1) + ':A' + str(row + 2)
        sheet.merge_range(merge_row, 'No.', head_cell_format)

        sheet.write(row,1, 'COM', head_cell_format_up)
        sheet.write(row+1,1, 'DATE', head_cell_format_down)
        ...
        sheet.merge_range(merge_row, 'ยอดก่อนภาษี', head_cell_format)

        merge_row = 'H' + str(row + 1) + ':H' + str(row + 2)
        sheet.merge_range(merge_row, 'สินค้า ' + commission_types[0].name, head_cell_format)

        merge_row = 'I' + str(row + 1) + ':I' + str(row + 2)
        sheet.merge_range(merge_row, 'สินค้า ' + commission_types[1].name, head_cell_format)

        merge_row = 'J' + str(row + 1) + ':J' + str(row + 2)
        sheet.merge_range(merge_row, 'สินค้าปกติ', head_cell_format)

        merge_row = 'K' + str(row + 1) + ':K' + str(row + 2)
        sheet.merge_range(merge_row, 'สินค้า', head_cell_format)

        merge_row = 'L' + str(row + 1) + ':L' + str(row + 2)
        sheet.merge_range(merge_row, 'พนักงานเก็บเงิน', head_cell_format)

        merge_row = 'M' + str(row + 1) + ':M' + str(row + 2)
        ...
        for item in payment_lines:
            ...
            data_list_2 = [ total_amount_untax or '',
                            total_amount_commission_1_untax or '',
                            total_amount_commission_2_untax or '',
                            total_amount_no_com or '',]
            sheet.write_row(row,6, data_list_2,data_cell_number_format)
                    
        row += 1
        merge_row = 'B' + str(row + 1) + ':F' + str(row + 1)
        header_data = 'พนักงานขาย : ' + str(user_id.employee_id.name) if user_id.employee_id.name else 'พนักงานขาย : '
        sheet.merge_range(merge_row, header_data, data_cell_format_left)

        data_list_2 = [ total_amount_untax_sum_all or '',
                        total_amount_commission_1_untax_sum_all or '',
