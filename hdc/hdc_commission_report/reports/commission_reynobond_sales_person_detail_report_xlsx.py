...
class CommissionReynobondSalesPersonDetailXlsx(models.AbstractModel):
    _name = 'report.commission_reynobond_sales_person_detail_report_xlsx'
    _description = 'report.commission_reynobond_sales_person_detail_report_xlsx'
    ...
    def generate_xlsx_report(self, workbook, data, partners):
        ...
        data_list_header = [
                        'CODE : ' + str(user_id.employee_id.barcode) if user_id.employee_id.barcode else 'CODE : ',
                        ' ',
                        ' ',
                        ' ',
                        ' ',
        ...
        sheet.merge_range('E3:F3', 'พนักงานขาย : ' + user_id.employee_id.name if user_id.employee_id.name else 'พนักงานขาย : ', top_cell_format)
        ...
        sheet.merge_range('G2:I2', '( รับชำระ' + ' ' + str(day_receipted_date_form) + '/' + str(month_receipted_date_form) + '/' + str(year_receipted_date_form) + ' ถึง ' + str(day_receipted_date_to) + '/' + str(month_receipted_date_to) + '/' + str(year_receipted_date_to) + ' )', top_cell_format)
        row = 5
        ...
        sheet.write(row,1, 'Date', head_cell_format_up)
        sheet.write(row+1,1, 'Rec.', head_cell_format_down)

        sheet.write(row,2, 'INV.', head_cell_format_up)
        sheet.write(row+1,2, 'Date', head_cell_format_down)

        merge_row = 'D' + str(row + 1) + ':D' + str(row + 2)
        ...
        sheet.write(row+1,7, commission_types[0].name or 'GIESSE', head_cell_format_down)
        ...
        total_amount_commission_untax = 0
        total_amount_commission_untax_sum_all = 0
        total_amount_normal_untax = 0
        ...
        for item in payment_lines:
            ...
            sheet.write_row(row,4, data_list_1_2,data_cell_format_left)
            data_list_2 = [ total_amount_untax or '',
                            total_amount_commission_untax or '',
                            total_amount_normal_untax or '',]
