...
class CommissionMarketDetailXlsx(models.AbstractModel):
    _name = 'report.commission_market_detail_report_xlsx'
    _description = 'report.commission_market_detail_report_xlsx'
    _inherit = 'report.report_xlsx.abstract'

    def generate_xlsx_report(self, workbook, data, partners):

        settle_commissions_id = partners.settle_commissions_id
        invoice_lines = settle_commissions_id.invoice_lines
        commission_date = settle_commissions_id.commission_date
        invoice_date_form = settle_commissions_id.invoice_date_form
        invoice_date_to = settle_commissions_id.invoice_date_to
        ...
        sheet.merge_range('B2:C2', 'คอมมิชชั่น เดือน' + ' ' + str(text_month_commission_date) + ' ' + str(year_commission_date), top_cell_format)
        sheet.merge_range('B3:C3', 'ทีม' + team_id.name, top_cell_format)
        ...
        sheet.merge_range('D2:E2', '( วันที่' + ' ' + str(day_invoice_date_form) + '/' + str(month_invoice_date_form) + '/' + str(year_invoice_date_form) + ' ถึง ' + str(day_invoice_date_to) + '/' + str(month_invoice_date_to) + '/' + str(year_invoice_date_to) + ' )', top_cell_format)
        ...
        sheet.write(row,0, 'Invoice Date', head_cell_format_up)
        sheet.write(row+1,0, 'Date', head_cell_format_down)

        sheet.write(row,1, 'SPE Invoice', head_cell_format_up)
        sheet.write(row+1,1, 'No.', head_cell_format_down)

        sheet.write(row,2, 'รายชื่อลูกค้า', head_cell_format_up)
        sheet.write(row+1,2, '', head_cell_format_down)

        sheet.write(row,3, 'รายการสินค้า', head_cell_format_up)
        sheet.write(row+1,3, '', head_cell_format_down)

        sheet.write(row,4, 'มูลค่า', head_cell_format_up)
        sheet.write(row+1,4, '(รวมภาษี)', head_cell_format_down)

        sheet.write(row,5, 'มูลค่า', head_cell_format_up)
        sheet.write(row+1,5, '(ก่อนภาษี)', head_cell_format_down)
        ...
        for item in invoice_lines:
            ...
            data_list_1_2 = [item.partner_id.name or '',
                            item.product_id.name or '',
                            ]
                
            sheet.write_row(row,0, data_list_1,data_cell_format)
            sheet.write_row(row,2, data_list_1_2,data_cell_format_left)
        ...
        sheet.write_row(row,4, data_list_2,data_cell_number_format_sum)
