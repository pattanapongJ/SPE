...
class WizardQuotationChangeTotal(models.TransientModel):
    _name = "wizard.quotation.change.total"
    _description = 'wizard quotation change total'

    quotation_id = fields.Many2one('quotation.order', string='Sale Order')
    ...
    def update_quotation_total(self):
        ...
        for field_name, value in rounding_fields.items():
            if value:
                ...
                setattr(self.quotation_id, field_name, value)
            else:
                setattr(self.quotation_id, field_name, False)
