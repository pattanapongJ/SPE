...
class PartnerXlsx(models.AbstractModel):
    _name = 'report.quotation_project_department_xlsx'
    _description = 'report.quotation_project_department_xlsx'
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        if department:
            department_ids = department.ids  # ดึง ID ของแผนก
        ...
        # report name
        report_name = ("Quotation Project Department")
        sheet = workbook.add_worksheet(report_name)
        sheet.merge_range('A1:U1', 'SPE Quotation Project (Department)', top_cell_format)
        sheet.merge_range('A2:U2', 'Saeng Charoen Group', top_cell_format)
        sheet.set_row(0, 20)
        sheet.set_row(1, 15)
        sheet.set_row(2, 15)
        ...
        for item in qo_id:
            ...
            sheet.merge_range(row,1,row,2, 'Customer', head_sub_cell_format)
            ...
            sheet.merge_range(row,9,row,11, 'Status', head_sub_cell_format)
            sheet.merge_range(row,12,row,13, 'Customer Req', head_sub_cell_format)
            sheet.merge_range(row,14,row,20, '', head_sub_cell_format)

            row +=1
            sheet.write(row,0, '', head_sub_cell_format)
            sheet.merge_range(row,1,row,2, '', head_sub_cell_format)
            sheet.merge_range(row,3,row,4, 'ExpDate', head_sub_cell_format)
            sheet.merge_range(row,5,row,6, 'Include', head_sub_cell_format)
            sheet.merge_range(row,7,row,8, 'Sales Spec', head_sub_cell_format)
            sheet.merge_range(row,9,row,11, 'Ramark', head_sub_cell_format)
            sheet.merge_range(row,12,row,13, '', head_sub_cell_format)
            sheet.merge_range(row,14,row,20, '', head_sub_cell_format)
            row +=1
            sheet.write(row,0, '', head_sub_cell_format)
            sheet.merge_range(row,1,row,2, '', head_sub_cell_format)
            sheet.merge_range(row,3,row,4, 'จำนวน/หน่วย', head_sub_cell_format)
            sheet.merge_range(row,5,row,6, 'ราคาเสนอลูกค้า', head_sub_cell_format)
            sheet.merge_range(row,7,row,8, 'Discount percent', head_sub_cell_format)
            sheet.merge_range(row,9,row,11, 'จำนวนเงิน', head_sub_cell_format)
            sheet.merge_range(row,12,row,13, '', head_sub_cell_format)
            sheet.write(row,13, 'transfer 24', head_sub_cell_format_warp)
            sheet.write(row,14, 'Transfer อื่น', head_sub_cell_format)
            sheet.write(row,15, 'ค้างรับ', head_sub_cell_format)
            sheet.write(row,16, 'ค้างส่ง', head_sub_cell_format)
            sheet.write(row,17, 'Physical Inv', head_sub_cell_format_warp)
            sheet.write(row,18, 'RSP-PRO', head_sub_cell_format)
            sheet.write(row,19, '02-SP-PRO', head_sub_cell_format)
            sheet.write(row,20, '02-SPวันที่เริ่มต้น/สิ้นสุด', head_sub_cell_format)
            ...
            sheet.write(row,0, item.name or '', head_sub_cell_format_top_border)
            sheet.merge_range(row,1,row,2, item.partner_id.name or '', head_sub_cell_format_top_border)
            ...
            sheet.merge_range(row,7,row,8, item.user_id.name, head_sub_cell_format_top_border)
            sheet.merge_range(row,9,row,11, item.state or '', head_sub_cell_format_top_border)
            sheet.merge_range(row,12,row,13, item.partner_id.name or '', head_sub_cell_format_top_border)
            sheet.merge_range(row,14,row,20, '', head_sub_cell_format_top_border)

            row +=1
            sheet.write(row,0, '', head_sub_cell_format_no_border)
            sheet.merge_range(row,1,row,2, '', head_sub_cell_format_no_border)
            ...
            sheet.merge_range(row,5,row,6, tax_names or '', head_sub_cell_format_no_border)
            sheet.merge_range(row,7,row,8, item.sale_spec.name or '', head_sub_cell_format_no_border)
            sheet.merge_range(row,9,row+1,11, item.remark or '', head_sub_cell_format_no_border)
            sheet.merge_range(row,12,row,13, '', head_sub_cell_format_no_border)
            sheet.merge_range(row,14,row,20, '', head_sub_cell_format_no_border)
            ...
            for i, line in enumerate(item.quotation_line):
                ...
                sheet.merge_range(row,1,row,2, line.product_id.name or '',data_cell_format_left)
                ...
                sheet.merge_range(row,7,row,8, line.sub_discount or '',data_cell_format_left) 
                sheet.merge_range(row,9,row,10, line.price_total or '',data_cell_format_left) 
                sheet.merge_range(row,11,row,12, '',data_cell_format_left) 
                ...
                sheet.write_row(row,13, [transfer24_dict.get(line.product_id.id, 0)],data_cell_format_left)
                sheet.write_row(row,14, [transfer_other_dict.get(line.product_id.id, 0)],data_cell_format_left)
                sheet.write_row(row,15, [batch_dict.get(line.product_id.id, 0)],data_cell_format_left)
                sheet.write_row(row,16, [picking_list_dict.get(line.product_id.id, 0)],data_cell_format_left)
                sheet.write_row(row,17, [physical_inv_dict.get(line.product_id.id, {}).get('total_amount', 0)],data_cell_format_left)
                sheet.write_row(row,18, ['RSP-PRO'],data_cell_format_left)
                sheet.write_row(row,19, ['02-SP-PRO'],data_cell_format_left)
                sheet.write_row(row,20, ['02-เริ่มสิ้น'],data_cell_format_left)
