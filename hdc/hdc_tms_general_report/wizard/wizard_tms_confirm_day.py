...
class WizardTmsDay(models.TransientModel):
    ...
    # billing_status = fields.Selection([('none', 'None'), ('to_billing', 'To Billing'), ('to_finance', 'To Finance'), ], string = "Billing Status", default = 'none')
    ...
    def print_pdf(self):
        ...
        return self.env.ref('hdc_tms_general_report.tms_report_day_view').report_action(self, data=data)
    ...
    def print_excel(self):
        ...
        worksheet.write(0, 0, 'รายงานจัดส่งสินค้าตามผู้จัดส่ง-แบบสรุป (รายวัน)') # row, column, value
        ...
        for index, doc in enumerate(distribution_deli_notes):
            ...
            invoice_line = doc.invoice_line_ids
            if self.tms_remark:
                invoice_line = invoice_line.filtered(lambda x: x.tms_remark == self.tms_remark)
            if doc.driver_id.name:
                worksheet.write(row_num, 1, doc.driver_id.name)
            worksheet.write(row_num, 2, len(invoice_line))
            total_amount = sum(line.amount_total for line in invoice_line)
        ...
        worksheet.write(row_num, 3, sum_total)
