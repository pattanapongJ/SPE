...
class ReportTmsMonthPaper(models.AbstractModel):
    _name = 'report.hdc_tms_general_report.tms_month_report'
    _description = 'Report TMS Month'
    ...
    def _get_report_values(self, docids, data=None):
        ...
        for rec in distribution_deli_notes:
            driver_name = rec.driver_id.name or ''
            quantity = len(rec.invoice_line_ids)
            if data['tms_remark']:
                quantity = len(rec.invoice_line_ids.filtered(lambda x: x.tms_remark == data['tms_remark']))

            if driver_name in driver_quantity:
                driver_quantity[driver_name] += quantity
            else:
                driver_quantity[driver_name] = quantity

        sum_count = 0

        for name, quantity in driver_quantity.items():        
            list_driver.append({'name': name, 'quantity': quantity})
            sum_count += quantity
