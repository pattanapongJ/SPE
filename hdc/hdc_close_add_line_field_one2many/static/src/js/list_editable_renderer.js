...
FormController.include({
    _onFieldChanged: function (event) {
        ...
        console.log("Field value changed to1: ", fieldName.rma_reason_id);
        if (fieldName.rma_reason_id) {
            // Custom onchange logic here
            console.log("Field value changed to2: ", fieldName.rma_reason_id);
...
ListRenderer.include({
    ...
    init: function (parent, state, params) {
        ...
        this.parent_model = parent.model;
        this.recordData_value = parent.recordData;
        this.recordData_state = ""
        if (this.recordData_value){
            this.recordData_state = this.recordData_value.state
        }
        // if selected product is true, we will add Add a line
        this.recordData_is_select_product_receipt = ""
        if (this.recordData_value){
            this.recordData_is_select_product_receipt = this.recordData_value.is_select_product_receipt
        }
        ...
        if (this.parent_model == 'sale.order'){
            this.creates = [];
            this.arch.children.forEach(child => {
                // close add a product when if sale.order == ['sale','done','cancel']
                if (this.parent_model == 'sale.order' && ['sale','done','cancel'].includes(this.recordData_state)){
                    return this.creates.push({});
                }
                ...
                child.children.forEach(child => {
                    ...
                    console.log('creates before Add context', this.creates)
                    this.creates.push({
                        context: child.attrs.context,
                        string: child.attrs.string,
                    });
                    console.log('creates after Add context', this.creates)
                });
            });
        }
        else{
            this.creates = [];
            this.arch.children.forEach(child => {
                if (this.parent_model == 'crm.claim.ept'){
                    this.creates = [{context: undefined, string: 'Add a product'},
                                    {context: "{'default_display_type': 'line_section'}", string: 'Add a section'},
                                    {context: "{'default_display_type': 'line_note'}", string: 'Add a note'}]
                }
                else{
                    this.creates.push({
                        context: child.attrs.context,
            ...
            // this.creates = [];
            // this.arch.children.forEach(child => {
            //     // close add a product when if crm.claim.ept == is_select_product_receipt == true
            //     if (this.parent_model == 'crm.claim.ept' && [false].includes(this.recordData_is_select_product_receipt)){
            //         return this.creates.push({});
            //     }
            //     if (child.tag !== 'control') {
            //         return;
            ...
            // console.log('creates before Add a line', this.creates)
            // if (this.creates.length === 0) {
            //     this.creates.push({
            //         string: _t("Add a line"),
            //     });
            // }
            // console.log('creates after Add a line', this.creates)
