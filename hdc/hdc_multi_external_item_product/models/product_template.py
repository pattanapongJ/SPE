...
class ProductTemplate(models.Model):
    ...
    multi_ext_product_available = fields.Integer(string="Multi External Item Product", compute='_compute_multi_ext_product')

    def _compute_multi_ext_product(self):
        for line in self:
            multi_ext_product_count = self.env['multi.external.product'].search_count([('product_tmpl_id', '=',self.id)]) 
            line.update({   
