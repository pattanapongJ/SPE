...
class DistributionDeliveryNoteLine(models.Model):
    ...
    def write(self, vals):
        """Override write method to update invoice status when delivery_status or billing_status changes"""
        result = super(DistributionDeliveryNoteLine, self).write(vals)
        
        # Check if delivery_status or billing_status has changed
        if 'delivery_status' in vals or 'billing_status' in vals:
            for record in self:
                if record.invoice_id and record.invoice_id.exists():
                    try:
                        # Use force_update_status for guaranteed update regardless of invoice state
                        delivery_status = vals.get('delivery_status')
                        billing_status = vals.get('billing_status')
                        
                        # Only update if values are different from current values
                        should_update = False
                        update_delivery = delivery_status is not None and delivery_status != record.delivery_status
                        update_billing = billing_status is not None and billing_status != record.billing_status
                        
                        if update_delivery or update_billing:
                            record.invoice_id.force_update_status(
                                delivery_status=delivery_status if update_delivery else None,
                                billing_status=billing_status if update_billing else None
                            )
                        ...
                        _logger.warning("Failed to update invoice status for line %s: %s" % (record.id, str(e)))
        
        return result

    @api.onchange('delivery_status')
    def _onchange_delivery_status(self):
        """Update invoice delivery_status when changed in invoice line"""
        if self.invoice_id and self.invoice_id.exists() and self.delivery_status:
            try:
                # Use force_update_status for guaranteed update regardless of invoice state
                self.invoice_id.force_update_status(delivery_status=self.delivery_status)
            except Exception as e:
                # Show warning to user but don't break the form
                import logging
                _logger = logging.getLogger(__name__)
                _logger.warning("Failed to update invoice delivery status: %s" % str(e))
                return {
                    'warning': {
                        'title': 'Status Update Warning',
                        'message': 'Could not update invoice delivery status: %s' % str(e)
                    }
                }

    @api.onchange('billing_status')
    def _onchange_billing_status(self):
        """Update invoice billing_status when changed in invoice line"""
        if self.invoice_id and self.invoice_id.exists() and self.billing_status:
            try:
                # Use force_update_status for guaranteed update regardless of invoice state
                self.invoice_id.force_update_status(billing_status=self.billing_status)
            except Exception as e:
                # Show warning to user but don't break the form
                import logging
                _logger = logging.getLogger(__name__)
                _logger.warning("Failed to update invoice billing status: %s" % str(e))
                return {
                    'warning': {
                        'title': 'Status Update Warning',
                        'message': 'Could not update invoice billing status: %s' % str(e)
                    }
                }

    @api.model
    def batch_update_invoice_status(self, line_ids, delivery_status=None, billing_status=None):
        """
        Batch update invoice status for multiple lines efficiently
        """
        if not line_ids:
            return
        ...
        for line in lines:
            if line.invoice_id and line.invoice_id.exists():
        ...
        for invoice_data in invoice_updates.values():
            try:
                invoice_data['invoice'].force_update_status(
                    delivery_status=invoice_data['delivery_status'],
                    billing_status=invoice_data['billing_status']
                )
            except Exception as e:
                import logging
                _logger = logging.getLogger(__name__)
                _logger.warning("Failed to batch update invoice %s: %s" % (invoice_data['invoice'].name, str(e)))
