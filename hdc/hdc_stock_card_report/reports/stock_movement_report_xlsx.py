...
class StockMovementXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, partners):
        ...
        data_cell_format_product = workbook.add_format({
            'border': True,
            'align': 'top',
            'bg_color': '#9FC5e8'
        })
        data_cell_format_product.set_font_size(9)
        data_cell_format_product.set_align('vcenter')
        data_cell_format_product.set_font_name('Kanit')
        ...
        data_cell_number_format_product = workbook.add_format({
            'num_format': '#,##0.00',
            'border': True,
            'align': 'top',
            'bg_color': '#9FC5e8'
        })
        data_cell_number_format_product.set_font_size(9)
        data_cell_number_format_product.set_align('vcenter')
        data_cell_number_format_product.set_font_name('Kanit')
        ...
        grandtotal_cell_format_total = workbook.add_format({
            'bold': True,
            'align': 'left',
            'bg_color': '#92D050',
            'top': True
        })
        grandtotal_cell_format_total.set_font_size(9)
        grandtotal_cell_format_total.set_align('vcenter')
        grandtotal_cell_format_total.set_font_name('Kanit')
        grandtotal_cell_number_format = workbook.add_format({
            'bold': True,
            'num_format': '#,##0.00',
            'border': True,
            'align': 'top'
        })
        grandtotal_cell_number_format.set_font_size(9)
        grandtotal_cell_number_format.set_align('vcenter')
        grandtotal_cell_number_format.set_font_name('Kanit')
        ...
        for i, product_ids in enumerate(product_id):
            ...
            if date_to and date_from:
                ...
                if warehouse_ids:
                    ...
                    [('date', '<=', date_to), ('date', '>=', date_from), ('state', '=', 'done'), ('warehouse_id', '=', warehouse_ids.ids),
                ...
                if warehouse_ids:
                    stock_move_id = self.env['stock.move'].search(
                        [('state', '=', 'done'), ('product_id', '=', product_ids.id), ('warehouse_id', 'in', warehouse_ids.ids), ('location_id.usage', 'not in', ('internal', 'transit')), ('location_dest_id.usage', 'in', ('internal', 'transit'))], order="date")
            ...
            if stock_move_id:
                ...
                res_product = product_ids._compute_quantities_dict(lot_id=None, owner_id=None, package_id=None,to_date=date_from)
                qty_available = res_product[product_ids.id]['qty_available']
                if warehouse_ids:
                    qty_available = 0
                    for warehouse in warehouse_ids:
                        res_product_warehouse = product_ids._compute_quantities_dict_warehouse(lot_id=None, owner_id=None, package_id=None,to_date=date_from,warehouse_id=warehouse)
                ...
                if first_product == False:     
                    sheet.write(row,9, "TOTAL", grandtotal_cell_format_total)

                    merge_row = 'K' + str(row + 1) + ':L' + str(row + 1)
                    sheet.merge_range(merge_row, '', grandtotal_cell_number_format)

                    merge_row1 = 'M' + str(row + 1) + ':N' + str(row + 1)
                    sheet.merge_range(merge_row1, internal_total_qty, grandtotal_cell_number_format)

                    merge_row2 = 'O' + str(row + 1) + ':P' + str(row + 1)
                    sheet.merge_range(merge_row2, transit_total_qty, grandtotal_cell_number_format)
                    
                    sheet.write(row,16, amount_total or 0, grandtotal_cell_number_format)
                    sheet.write(row,17, grand_uom_name, grandtotal_cell_format)   
                    row += 1
                    first_product == True   
                ...
                sheet.write_row(row,0, data_list, data_cell_format_product) 

                merge_row = 'K' + str(row + 1) + ':L' + str(row + 1)
                sheet.merge_range(merge_row, before_total or 0, data_cell_number_format_product)

                merge_row1 = 'M' + str(row + 1) + ':N' + str(row + 1)
                sheet.merge_range(merge_row1, '', data_cell_number_format_product)

                merge_row2 = 'O' + str(row + 1) + ':P' + str(row + 1)
                sheet.merge_range(merge_row2, '', data_cell_number_format_product)
                ...
                for stock_move_ids in stock_move_id:
                    ...
                    grand_uom_name = product_ids.uom_id.name or ''
