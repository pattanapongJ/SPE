...
class FinancialDetailSummaryXlsx(models.AbstractModel):
    _name = 'report.financial_detail_report_summary_xlsx'
    _description = 'report.financial_detail_report_summary_xlsx'
    ...
    def generate_xlsx_report(self, workbook, data, partners):
        ...
        sheet.write(1, 2, 'SPE Stock card - Financial (Summary)',top_cell_format)
        ...
        sheet.merge_range('E9:G10', 'ยอดยกมา', head_cell_format)
        sheet.write(10,4, 'จำนวน', head_sub_cell_format)
        sheet.write(10,5, 'ราคา-หน่วย', head_sub_cell_format)
        sheet.write(10,6, 'มูลค่า', head_sub_cell_format)

        sheet.merge_range('H9:J10', 'รับเข้า', head_cell_format)
        sheet.write(10,7, 'จำนวน', head_sub_cell_format)
        sheet.write(10,8, 'ราคา-หน่วย', head_sub_cell_format)
        sheet.write(10,9, 'มูลค่า', head_sub_cell_format)

        sheet.merge_range('K9:M10', 'จ่ายออก', head_cell_format)
        sheet.write(10,10, 'จำนวน', head_sub_cell_format)
        sheet.write(10,11, 'ราคา-หน่วย', head_sub_cell_format)
        sheet.write(10,12, 'มูลค่า', head_sub_cell_format)

        sheet.merge_range('N9:P10', 'ยอดคงเหลือ', head_cell_format)
        sheet.write(10,13, 'จำนวน', head_sub_cell_format)
        sheet.write(10,14, 'ราคา-หน่วย', head_sub_cell_format)
        sheet.write(10,15, 'มูลค่า', head_sub_cell_format)
                ...
                for locat in financial_detail_results_location:
                    ...
                    for data in financial_detail_results_product:
                        if data["location_dest_id"] == locat:
                            ...
                            internal_total_price = internal_total_price + data["in_price"]
                            transit_total_qty = transit_total_qty + data["qty_out"]
                            transit_total_price = transit_total_price + data["out_price"]           
                    ...
                    amount_total_price = amount_total * product_ids.standard_price

                    internal_unit_price = 0
                    transit_unit_price = 0
                    if internal_total_qty > 0:
                        internal_unit_price = internal_total_price / internal_total_qty
                    if transit_total_qty > 0:
                        transit_unit_price = transit_total_price / transit_total_qty
                    row += 1
                    data_list = [
                                product_ids.default_code or "",
                                product_ids.name or "",]
                    before_total_price = qty_available_summary * product_ids.standard_price
                    before_standard_price = False
                    amount_standard_price = False
                    if qty_available_summary > 0:
                        before_standard_price = product_ids.standard_price
                    if amount_total > 0:
                        amount_standard_price = product_ids.standard_price
                    data_list_2 = [ qty_available_summary or "",
                                before_standard_price or "",
                                before_total_price or "",
                                internal_total_qty or "",
                                internal_unit_price or "",
                                internal_total_price or "",
                                transit_total_qty or "",
                                transit_unit_price or"",
                                transit_total_price or "",
                                amount_total or "",
                                amount_standard_price or "",
                    ...
                    sheet.write_row(row,4, data_list_2,data_cell_number_format)
                    row += 1
                    data_list_3 = [ 
                        internal_total_qty or 0,
                        "",
                        internal_total_price or 0,
                        transit_total_qty or 0,
                        "",
                        transit_total_price or 0,
                        amount_total or 0,
                        "",
                        amount_total_price or 0,]
                    sheet.write_row(row,7, data_list_3,data_cell_number_format)
                    sheet.write(row,5, "TOTAL", data_cell_format)
                    grand_before_total = grand_before_total + qty_available_summary
                    grand_before_total_price = grand_before_total_price + before_total_price
                    in_total = in_total + internal_total_qty
                    in_total_price = in_total_price + internal_total_price
                    out_total = out_total + transit_total_qty
                    out_total_price = out_total_price + transit_total_price
                    grand_amount_total = grand_amount_total + amount_total 
                    grand_amount_price_total = grand_amount_price_total + amount_total_price
        
        row += 1

        grand_before_unit_price = 0
        in_unit_price = 0
        out_unit_price = 0
        grand_amount_unit_price = 0
        if grand_before_total > 0:
            grand_before_unit_price = grand_before_total_price / grand_before_total
        if in_total > 0:
            in_unit_price = in_total_price / in_total
        if out_total > 0:
            out_unit_price = out_total_price / out_total
        if grand_amount_total > 0:
            grand_amount_unit_price = grand_amount_price_total / grand_amount_total
        ...
        data_list = [
                    ...
                    "",
                    "",
                    grand_before_total or 0,
                    grand_before_unit_price or 0,
                    grand_before_total_price or 0,
                    in_total or 0,
                    in_unit_price or 0,
                    in_total_price or 0,
                    out_total or 0,
                    out_unit_price or 0,
                    out_total_price or 0,
                    grand_amount_total or 0, 
                    grand_amount_unit_price or 0, 
                    grand_amount_price_total or 0, 
                    ]
