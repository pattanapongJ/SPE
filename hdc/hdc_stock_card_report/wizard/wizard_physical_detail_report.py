...
class WizardPhysicalDetailReceiveReport(models.TransientModel):
    _name = 'wizard.physical.detail.report'
    _description = 'Wizard PhysicalDetail'
    ...
    def button_export_html(self):
        self.ensure_one()
        action = self.env.ref("hdc_stock_card_report.action_report_physical_detail_report_html")
        vals = action.sudo().read()[0]
        context = vals.get("context", {})
        if context:
            context = safe_eval(context)
        model = self.env["report.physical.detail.report"]
