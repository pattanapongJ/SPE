...
class ClaimProcessWizard(models.TransientModel):
    _name = 'purchase.claim.process.wizard'
    _description = 'Purchase Wizard to process claim lines'

    claim_line_id = fields.Many2one('purchase.claim.line.ept', "Claim Line")
    ...
    reject_message_id = fields.Many2one("purchase.claim.reject.message", "Reject Reason")
    ...
    def default_get(self, default_fields):
        ...
        if self._context.get('active_model') == 'purchase.crm.claim.ept':
            ...
            line = self.env['purchase.claim.line.ept'].search([('id', '=', self._context.get('active_id'))])
            res['claim_line_id'] = line.id
            res['state'] = line.purchase_claim_id.state if line.purchase_claim_id else line.ticket_id.state
