...
class CrmClaimEpt(models.Model):
    ...
    is_rma_without_incoming = fields.Boolean(string="Is RMA Without Delivery")
    ...
    reject_message_id = fields.Many2one("purchase.claim.reject.message", string="Reject Reason", copy=False)
    new_purchase_id = fields.Many2one('purchase.order', string='New Purchase Order', copy=False)
    ...
    picking_id = fields.Many2one('stock.picking', string='Receipt Order')
    ...
    refund_invoice_id = fields.Many2one('account.move', string='Refund Invoice', copy=False)
    claim_lot_ids = fields.Many2many('stock.production.lot', compute='_compute_lot_ids')

    repair_order_ids = fields.One2many('repair.order', 'purchase_claim_id', string='Repairs')
    claim_line_ids = fields.One2many("purchase.claim.line.ept", "purchase_claim_id", string="Return Line")
    ...
    rma_reason_journal_id = fields.Many2one('account.journal', domain=[("type", "=", "purchase"),("show_in_credit_note", "=", True)], string="Journal")
    is_rma_without_receipt = fields.Boolean(string="Is RMA Without Receipt")

    @api.onchange('rma_reason_id','claim_line_ids')
    ...
    def _data_picking(self):
        picking = []
        # purchase = []
        get_stock_move = self.env['stock.picking'].search([('picking_type_code', 'in', ['outgoing', 'incoming'])])
        for line in get_stock_move:
            claim_lines = []
            if line:
                claim_line_data = []
                for move_id in line.move_lines:
                    ...
                    created claim"""
                    previous_claimline_ids = self.env['purchase.claim.line.ept'].search([
                ...
                if len(claim_line_data) > 0:
                    picking.append(line.id)
        return picking

    @api.onchange('date')
    def _purchase_claim_id(self):
        picking_can_be_use = self._data_picking()
        return {"domain": {
            "picking_id": [('id', 'in', picking_can_be_use), ('picking_type_code', '=', 'incoming'), ('purchase_id', '!=', False)],
        }}

    @api.depends('picking_id', 'is_rma_without_receipt')
    def _compute_move_product_ids(self):
        for record in self:
            if record.is_rma_without_receipt:
                # all_products = self.env['product.product'].search([]).ids # ปิดการทำงานไป มันนาน ทำ lazy domain เอา 2 fields
                all_products = []
                record.move_product_ids = [(6, 0, all_products)]
            elif record.picking_id:
                products = record.picking_id.mapped('move_lines').mapped('product_id')
                record.move_product_ids = [(6, 0, products.ids)]
            else:
                record.move_product_ids = [(6, 0, [])]

    @api.onchange('picking_id')
    def onchange_picking_id(self):
        """This method is used to set default values in the RMA base on delivery changes."""
        claim_lines = []
        self._purchase_claim_id()
        if self.picking_id:
            returned_qty = 0
            ...
            # return {"domain": {"purchase_id": [('id', '=', self.picking_id.purchase_id.id)]}}
        else:
            self.partner_id = False
            self.partner_phone = False
            self.email_from = False
            self.partner_delivery_id = False
        return {"domain": {"purchase_id": self._domain_picking_claim_id()}}
    ...
    def _domain_picking_claim_id(self):
        purchase = []
        picking_can_be_use = self._data_picking()
        get_purchase_order = self.env['purchase.order'].search([('state', '=', 'purchase')])
        for line in get_purchase_order:
            purchase_done = False
            if line:
                for picking_id in line.picking_ids:
                    for pick_can_be_use in picking_can_be_use:
                        if pick_can_be_use == picking_id.id and picking_id.picking_type_code == 'incoming' and picking_id.state == 'done':
                            purchase_done = True
                if purchase_done:
                    purchase.append(line.id)
        return [('id', 'in', purchase)]

    @api.depends('claim_line_ids')
    def _compute_lot_ids(self):
        """select lot ids based on picking's move line"""
        for purchase_claim_id in self:
            move_lines = purchase_claim_id.picking_id.move_lines
            if move_lines and move_lines.move_line_ids.lot_id:
                purchase_claim_id.claim_lot_ids = move_lines.move_line_ids.lot_id
            else:
                purchase_claim_id.claim_lot_ids = [(6, 0, [])]
    ...
    def create(self, vals):
        ...
        if vals.get('code', 'New') == 'New':
            vals['code'] = self.env['ir.sequence'].next_by_code('purchase.crm.claim.ept')
    ...
    def change_field_based_on_picking(self):
        ...
        self.partner_delivery_id = purchase_id and purchase_id.partner_id and \
                                   purchase_id.partner_id.id \
                                   or self.picking_id.rma_purchase_id \
                                   and self.picking_id.rma_purchase_id.partner_shipping_id \
                                   and self.picking_id.rma_purchase_id.partner_shipping_id.id \
                                   or False
            ...
            for line_id in move_id.move_line_ids:
                if line_id.lot_id:
                    claim_lines.append((0, 0, {
                        'product_id_r': move_id.product_id.id, 'product_id': move_id.product_id.id, 'quantity': line_id.qty_done, 'move_id': move_id.id,
                        'serial_lot_ids': line_id.lot_id
                        }))
                else:
                    claim_lines.append((0, 0, {
                        'product_id_r': move_id.product_id.id, 'product_id': move_id.product_id.id, 'quantity': move_id.quantity_done, 'move_id': move_id.id
                        }))
        return claim_lines
    ...
    def check_retutn_qty(returned_qty, move_id, claim_lines):
        ...
        if returned_qty < move_id.quantity_done:
            ...
            if qty > 0:
                for line_id in move_id.move_line_ids:
                    if line_id.lot_id:
                        claim_lines_std.append({
                                                   'product_id_r': move_id.product_id.id, 'product_id': move_id.product_id.id, 'done_qty': qty,
                                                   'qty': line_id.qty_done, 'move_id': move_id.id,
                                                   'lot_id': line_id.lot_id[0].id
                                                   })
                    else:
                        claim_lines.append((0, 0, {
                            'product_id_r': move_id.product_id.id, 'product_id': move_id.product_id.id, 'done_qty_update': qty, 'quantity': qty,
                            'move_id': move_id.id
                            }))
                        
                ##########################
                        
                # ซ่อนไว้ เพราะ function นี้ ไม่ได้รับ 'claim_lines_search' เข้ามา แต่มี function ที่คล้ายกันใน rma_ept ที่รับ 'claim_lines_search' เอามาใช้ เลยได้ใช้ code ด้านล่าง

                # if claim_lines_search:
                #     for line_search in claim_lines_search:
                #         for line in claim_lines_std:
                #             if line["product_id"] == line_search['product_id'] and line["lot_id"] == line_search[
                #                 'lot_id']:
                ...
                for line_id in claim_lines_std:
                    if line_id['qty'] > 0:
                        claim_lines.append((0, 0, {
                            'product_id_r': line_id['product_id_r'], 'product_id': line_id['product_id'], 'done_qty_update': line_id['done_qty'],
                            'quantity': line_id['qty'], 'move_id': line_id['move_id'],
                            'serial_lot_ids': [line_id['lot_id']]
                            }))
        return claim_lines
    ...
    def search_mail_data(self, mail_obj, result, partner_id):
        ...
        return mail_obj.search([
            ('res_id', '=', res_id), ('res_model', '=', 'purchase.crm.claim.ept'),
    ...
    def create_contact_partner(self):
        ...
        return {
            ...
            'view_mode':'form',
            'res_model':'purchase.create.partner.delivery.address.ept',
    ...
    def action_view_repair_orders(self):
        ...
        return {
            ...
            'view_mode':'tree,form',
            'domain':[('purchase_claim_id', '=', self.id)],
    ...
    def create_return_picking(self, claim_lines=False):
        ...
        vals = {'picking_id':self.return_picking_id.id if claim_lines else self.picking_id.id, 'remark': "RETURN PURCHASE RMA"}
        ...
        new_picking_id, pick_type_id = return_picking_wizard._create_rma_supplier_returns()
        if claim_lines:
    ...
    def set_as_internal_picking(self, location_id, claim_lines, new_picking_id):
        ...
        internal_picking_id.write({'purchase_claim_id':self.id})
    ...
    def approve_claim(self):
        ...
        is_rma_without_receipt = self.is_rma_without_receipt
        ...
        if not is_rma_without_receipt:
            for line in self.claim_line_ids:
                ...
                prev_claim_lines = line.search([('move_id', '=', line.move_id.id),
                                                ('purchase_claim_id.state', 'in', ['process',
                                                                        'approve',
                                                                        'close'])])
        ...
        elif is_rma_without_receipt:
            self.write({'state':'process'})
        else:
            return_picking_id = self.create_return_picking()
            if return_picking_id:
                return_picking_id.do_unreserve()
                return_picking_id.write({'purchase_claim_id':self.id})
    ...
    def action_rma_send_email(self):
        ...
        email_template = self.env.ref('hdc_rma_ept_purchase.mail_rma_details_notification_ept', False)
    ...
    def reject_claim(self):
        ...
        return {
            ...
            'view_id':self.env.ref('hdc_rma_ept_purchase.view_purchase_claim_reject_ept').id,
            'type':'ir.actions.act_window',
    ...
    def act_supplier_invoice_refund_ept(self):
        ...
        if len(self.refund_invoice_ids) == 1:
            ...
            refun_invoice_action = {
                ...
                'domain':[('id', 'in', self.refund_invoice_id.ids)]
            }
    ...
    def act_new_so_ept(self):
        ...
        return {
            ...
            'type':'ir.actions.act_window',
            'res_id':self.new_purchase_id.id
        }
    ...
    def check_validate_claim(self):
        ...
        if not self.is_rma_without_receipt:
    ...
    def create_so(self, claim_lines):
        ...
        order_vals = self.prepare_purchase_order_values()
        purchase_order = self.env['purchase.order'].create(order_vals)

        purchase_order.onchange_partner_id()
        # purchase_order.onchange_partner_shipping_id()

        self.create_purchase_order_lines(purchase_order, claim_lines)
        self.write({'new_purchase_id':purchase_order.id})

    def create_purchase_order_lines(self, purchase_order, lines):
        """This method used to create a sale order line."""
        for line in lines:
            order_line_vals = self.prepare_purchase_order_line_values(purchase_order, line)
            order_line = self.env['purchase.order.line'].create(order_line_vals)
    ...
    @staticmethod
    def prepare_purchase_order_line_values(purchase_order, line):
    ...
    def create_do(self, claim_lines):
        ...
        for line in lines:
            if line.claim_type == 'replace_same_product':
                ...
                return_line = self.env['stock.return.picking.line'].create(return_line_values)
            elif line.claim_type == 'replace_other_product':
                move_id = self.env['stock.move'].search([
                    ('product_id', '=', line.product_id.id),
                    ('picking_id', '=', self.return_picking_id.id if claim_lines else
                    self.picking_id.id),
                ...
                return_line_values = {
                    'product_id': line.to_be_replace_product_id.id,
                    'quantity': line.to_be_replace_quantity,
                    'wizard_id': return_picking_wizard.id,
                    'move_id': move_id.id
                }
                return_line = self.env['stock.return.picking.line'].create(return_line_values)
    ...
    def _prepare_procurement_group_vals(self):
        ...
        return {
            ...
            'move_type':self.picking_id.move_type or 'direct',
        }
    ...
    def _prepare_procurement_values(self, group_id):
        ...
        return {
            'group_id':group_id,
            'warehouse_id':self.purchase_id.picking_type_id.warehouse_id,
            'partner_id':self.partner_delivery_id.id,
            'company_id':self.company_id,
            'rma_purchase_id':self.purchase_id.id,
        }
        ...
        if not self.purchase_id.invoice_ids:
            message = _(
                ...
                "<a href=# data-oe-model=purchase.order data-oe-id=%d>%s</a>") % (
                          self.purchase_id.id, self.purchase_id.name)
            ...
            new_product = []
            if self.claim_line_ids:
                for line_claim in self.claim_line_ids:
                    if line_claim.product_id and line_claim.claim_type in ['refund', 'replace_other_product']:
                        product_claim.append(({
                            'product_id': line_claim.product_id.id,
                            'return_qty': line_claim.return_qty
                        }))
            # self.purchase_id.action_create_invoice_rma_purchase(rma_id=self.id, product_claim=product_claim)
            invoice = self.env['account.move'].search([('id', 'in', self.purchase_id.invoice_ids.ids), ('state', '=', 'posted')])
            if not invoice:
                raise UserError(_("No posted invoice found. Please check the invoices in the purchase order again."))
            invoice_ref = invoice[0]

            if product_claim and invoice_ref:
                for invoice_line in invoice_ref.invoice_line_ids:
                    for product in product_claim:

                        if invoice_line.product_id.id == product['product_id']:
                            new_product.append(
                                (0, 0, {
                                    ...
                                    # 'wtvat': invoice_line.wtvat,
                                    'price_subtotal': invoice_line.price_subtotal
                                })
                            )
                if new_product:
                    journal_id = invoice_ref.journal_id.id
                    if self.rma_reason_journal_id:
                        journal_id = self.rma_reason_journal_id.id
                    refund_invoice_id = self.env['account.move'].create({
                        'ref': _('Reversal of: %(move_name)s', move_name=invoice_ref.name),
                        'date': time.strftime("%Y-%m-%d %H:%M:%S"),
                        'invoice_date': time.strftime("%Y-%m-%d %H:%M:%S"),
                        'journal_id': journal_id,
                        'invoice_payment_term_id': None,
                        'invoice_user_id': invoice_ref.invoice_user_id.id,
                        'move_type': 'in_refund',
                        'partner_id': invoice_ref.partner_id.id,
                        'invoice_line_ids': new_product,
                    })
                    self.write({'refund_invoice_id': refund_invoice_id.id})
            return True
    ...
    def action_rma_send(self):
        ...
        self.ensure_one()
        self.rma_send = True
        template = self.env.ref('hdc_rma_ept_purchase.mail_rma_details_notification_ept', False)
        compose_form = self.env.ref('mail.email_compose_message_wizard_form', False)
        ctx = {
            'default_model':'purchase.crm.claim.ept',
